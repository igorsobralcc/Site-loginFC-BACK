...
exports.create = async (req, res) => {
  ...
  Usuario.create(usuario)
    .then(data => {
      res.send({ email: data.email, função: data.funcao, status: data.status, token: generateToken({ id: data.id }) });
    })
}
...
exports.autenthicate = async (req, res) => {
  ...
  if (!await bcrypt.compare(senha, usuario.senha_hash))
      return res.status(400).send({ error: 'Senha inválido' })

  usuario.senha = undefined


  return res.send({ id: usuario.id, email: usuario.email, função: usuario.funcao, status: usuario.status, token: generateToken({ id: usuario.id }) })
}
...
exports.forgot = async (req, res) => {
  ...
  try {
    ...
    await Usuario.update({ passwordResetToken: token, passwordResetExpires: now }, {
      where:{
        id: emailFind.dataValues.id
    }})
    ...
    mailer.sendMail({
      ...
      if (err)
        return res.status(400).send({ error: 'não foi possivel recuperar a senha' })
...
exports.recover = async (req, res) => {
  ...
  try {
    ...
    if (token !== emailFind.dataValues.passwordResetToken)
      return res.status(400).send({ error: 'Token invalido' })

    const now = new Date()

    if (now > emailFind.dataValues.passwordResetExpires)
      return res.status(400).send({ error: 'Token expirou, gere um novo' })

    await Usuario.update({ senha: senha, passwordResetExpires: now }, {
      where: {
        id: emailFind.dataValues.id
      }
    })

    res.send({ ok: 'recuperado', token: generateToken({ id: emailFind.dataValues.id }) })

  } catch (error) {
    console.log("erro ", error)
    res.status(400).send({ error: 'não foi posivel resetar a senha, tente novamente' })
  }
}
...
exports.findAll = (req, res) => {
  ...
  Usuario.findAll({ where: condition })
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Aconteceu um erro ao consultar os usuarios."
      });
    });
};
...
exports.deleteAll = (req, res) => {
  Usuario.update({ status: false },{
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Aconteceu um erro ao deletar todos os usuarios."
      });
    });
};
...
exports.findAllAtivo = (req, res) => {
  Usuario.findAll({ where: { status: true } })
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Aconteceu um erro ao pegar todos os usuarios."
      });
    });
}
